...
NumericVector rcpp_pgdraw(NumericVector, NumericVector);

// [[Rcpp::export]]
NumericMatrix update_omega(int n, int K, List whichik, 
                                  NumericMatrix xbetak, NumericMatrix zgammak){
  ...
  NumericVector zg; 
  NumericVector draw; 
  NumericVector fillvector;
  
  for(int i = 0; i < n; ++i){
    
    idx = whichik(i); // list of integers 
    xb = xbetak.row(i); 
    zg = zgammak.row(i);
    c = xb[idx-1] + zg[idx-1]; // index starts at 0 

    draw = rcpp_pgdraw(b, c);
    fillvector = rep(0,K);
    
    for(int j = 0; j < idx.length(); ++j){
      fillvector(j) = draw(j);
    }
    omega(i,_) = fillvector;
  }
  return(omega);
}
...
NumericVector return_pgdraw(NumericVector b, NumericVector c){
  NumericVector ans = rcpp_pgdraw(b,c);
  return(ans);
}
